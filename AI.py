...
class AI(object):
    ...
    # def get_move(self):
    #     return self.get_best_new_board(self.get_future_boards(self.board, 1))
    ...
    def get_stats_of_board_after_move(self, board, piece):
        opposing_king_pos = board.get_white_king_pos(
        ) if self.color == 1 else board.get_black_king_pos()

        no_moves = board.no_valid_moves(abs(self.color - 1))

        available_moves = board.all_valid_moves(self.color)

        opposing_post_available_moves = board.all_valid_moves(
            abs(self.color - 1))

        opposing_post_king_moves = board.get(
            opposing_king_pos).get_valid_moves()
        ...
        opposing_post_attack_moves = board.all_attack_moves(
            abs(self.color - 1))
    ...
    def get_stats_of_board_before_move(self, board, piece):
        opposing_king_pos = board.get_white_king_pos(
        ) if piece.color == 1 else board.get_black_king_pos()
        ...
        opposing_pre_available_moves = board.all_valid_moves(
            abs(self.color - 1))

        opposing_pre_king_moves = board.get(
            opposing_king_pos).get_valid_moves()
    ...
    def get_best_move_of_piece(self, board, piece):
        ...
        for move in moves:
            ...
            limitation = (len(opposing_pre_available_moves) - len(
                opposing_post_available_moves)) / (len(opposing_pre_available_moves) + 0.25)

            limit_king = (len(opposing_pre_king_moves) - len(opposing_post_king_moves)
                          ) / (len(opposing_pre_king_moves) + 0.25)
            ...
            for move_made in board.moves_made[-10:]:
                if piece == move_made[1] and (move == move_made[-2:] or move == move_made[-4:-2]):
                    made_before += (15 /
                                    abs(move_made[0] - board.current_turn))
    ...
    def get_future_boards(self, board, future_steps, first_move = None):
        ...
        for ref_piece in all_self_pieces:
            piece = temp_board.grid[ref_piece.row][ref_piece.column]
            for move in piece.get_valid_moves():
                ...
                temp_board2 = Board()
                temp_board2.deep_copy(temp_board)
                temp_board2.refresh_pieces()
                for ref_piece2 in all_opposing_pieces:
                    other_piece = temp_board2.grid[ref_piece2.row][ref_piece2.column]
                    for move2 in other_piece.get_valid_moves():
                        ...
                        if (first_move == None):
                            boards += self.get_future_boards(
                                temp_board2, future_steps - 1, [piece, move])
                        else:
                            boards += self.get_future_boards(
                                temp_board2, future_steps - 1, first_move)
                        other_piece.undo_move(
                            origin_row2, origin_column2, temp2)
    ...
    def get_best_move(self, piece):
        ...
        for move in moves:
            ...
            promote = 0
            develop = 0
            opposing_king_pos = self.board.get_white_king_pos(
            ) if self.color == 1 else self.board.get_black_king_pos()

            self_pre_attacking_moves = self.board.all_attack_moves(self.color)

            opposing_pre_attack_moves = self.board.all_attack_moves(
                abs(self.color - 1))

            opposing_pre_available_moves = self.board.all_valid_moves(
                abs(self.color - 1))

            opposing_pre_king_moves = self.board.get(
                opposing_king_pos).get_valid_moves()
            ...
            opposing_post_available_moves = self.board.all_valid_moves(
                abs(self.color - 1))

            opposing_post_king_moves = self.board.get(
                opposing_king_pos).get_valid_moves()
            ...
            opposing_post_attack_moves = self.board.all_attack_moves(
                abs(self.color - 1))

            piece.undo_move(origin[0], origin[1], temp)
            self.board.kings_in_check()[self.color]

            openness = ((4 // self.board.current_turn) + 1) * \
                len(available_moves)
            ...
            if (piece.steps_taken <= 3):
                develop += self.board.current_turn / \
                    (25 * piece.steps_taken + 1)
            ...
            for move_made in self.board.moves_made[-10:]:
                if piece == move_made[1] and (move == move_made[-2:] or move == move_made[-4:-2]):
                    made_before += (15 /
                                    abs(move_made[0] - self.board.current_turn))
