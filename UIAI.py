...
class UI(object):
    ...
    def set_backgrounds(self):
        self.background_white = pyglet.image.SolidColorImagePattern(
            (255, 255, 255, 255)).create_image(self.scale // 8, self.scale // 8)
        self.background_gray = pyglet.image.SolidColorImagePattern(
            (75, 75, 75, 255)).create_image(self.scale // 8, self.scale // 8)
        self.background_select = pyglet.image.SolidColorImagePattern(
            (0, 175, 0, 255)).create_image(self.scale // 8, self.scale // 8)
        self.background_option_dark = pyglet.image.SolidColorImagePattern(
            (175, 175, 0, 255)).create_image(self.scale // 8, self.scale // 8)
        self.background_option_light = pyglet.image.SolidColorImagePattern(
            (225, 225, 0, 255)).create_image(self.scale // 8, self.scale // 8)
        self.background_check = pyglet.image.SolidColorImagePattern(
            (255, 0, 0, 255)).create_image(self.scale // 8, self.scale // 8)
        self.background_checkmate = pyglet.image.SolidColorImagePattern(
            (150, 0, 0, 255)).create_image(self.scale // 8, self.scale // 8)
        self.background_stalemate = pyglet.image.SolidColorImagePattern(
            (0, 0, 255, 255)).create_image(self.scale // 8, self.scale // 8)
        self.background_recent_light = pyglet.image.SolidColorImagePattern(
            (125, 125, 255, 255)).create_image(self.scale // 8, self.scale // 8)
        self.background_recent_dark = pyglet.image.SolidColorImagePattern(
            (75, 75, 255, 255)).create_image(self.scale // 8, self.scale // 8)
            ...
            for col in range(len(self.game.main_board.grid[r])):
                ...
                if (self.game.is_piece_selected and self.game.selected_piece != None and self.game.selected_piece in self.game.main_board.pieces):
                    if ([row, column] == [self.game.selected_piece.row, self.game.selected_piece.column]):
                        self.background_select.blit(
                            self.scale // 8 * col, self.scale // 8 * r)
                        ...
                        self.background_option_dark.blit(
                            self.scale // 8 * col, self.scale // 8 * r)
                        ...
                        self.background_option_light.blit(
                            self.scale // 8 * col, self.scale // 8 * r)
                    elif ([row, column] in self.game.recent_move and r % 2 == col % 2):
                        self.background_recent_dark.blit(
                            self.scale // 8 * col, self.scale // 8 * r)
                    elif ([row, column] in self.game.recent_move):
                        self.background_recent_light.blit(
                            self.scale // 8 * col, self.scale // 8 * r)
                    elif (r % 2 == col % 2):
                        self.background_gray.blit(
                            self.scale // 8 * col, self.scale // 8 * r)
                    else:
                ...
                if (game_over[0]):
                    if ((game_over[1] == 0 and [row, column] == self.game.main_board.get_white_king_pos()) or (game_over[1] == 1 and [row, column] == self.game.main_board.get_black_king_pos())):
                        self.background_checkmate.blit(
                            self.scale // 8 * col, self.scale // 8 * r)
                        ...
                        self.background_stalemate.blit(
                            self.scale // 8 * col, self.scale // 8 * r)
                    ...
                    self.background_check.blit(
                        self.scale // 8 * col, self.scale // 8 * r)

                if piece != None:
                    temp_sprite = pyglet.sprite.Sprite(
                        piece.image, col * self.scale // 8, r * self.scale // 8)
